  ...
  try {
    ...
    const plans: PlanCalendarData[] = await Promise.all(
      activeSubscriptions.map(async (sub, index) => {
        ...
        // Get display settings for color
        ...
        const progress =
          scheduleIds.length > 0 ? await repositories.progress.bulkGetProgress(sub.id, scheduleIds) : []
    ...
    return (
      <main data-testid="calendar-page" style={{ backgroundColor: '#F9F8F6', minHeight: '100vh' }}>
