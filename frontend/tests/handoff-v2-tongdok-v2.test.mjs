...
for (const chapter of [8]) {
  test('browsing an unscheduled chapter shows the raw off-plan response and rejects completion without writes', async () => {
    const runtime = tongdokRuntime({ query: { tongdok: 'true', plan: '7', schedule: '101' }, get: async params => rangeDetail(params) })
    const { useTongdokMode } = await loadTongdok(runtime); const mode = useTongdokMode(); mode.initTongdokMode()
    await mode.loadReadingDetail(7, 'exo', 1)
    assert.deepEqual((await mode.completeCurrentChapter('exo', 1)).persistedScheduleIds, [102])
    ...
    assert.equal(browsed.plan_date, undefined)
    assert.deepEqual(browsed.plan_detail, [])
    ...
    // 플랜에 없는 위치는 비통독처럼 표시된다: 고정된 일정 행·날짜·진행률이 아니라
    // 응답 그대로(빈 일정)가 노출되고, 선택된 플랜/일정 식별자만 유지된다.
    assert.equal(mode.isOffPlanDetail(), true)
    assert.deepEqual(mode.readingDetailResponse.value.data.plan_detail, [])
    assert.equal(mode.readingDetailResponse.value.data.plan_name, 'Plan 7')
    ...
    assert.equal(mode.getScheduleDate(), null)
    assert.equal(mode.getTongdokProgress('gen', chapter), null)
    assert.deepEqual(mode.getScheduleRows(), [])
    assert.deepEqual(mode.getCurrentSectionChapters('gen'), [])
    assert.equal(mode.getFullScheduleRange(), '')
    assert.equal(mode.getTongdokScheduleRange('gen', chapter), null)
    ...
    assert.equal(mode.getAudioLink('gen', chapter), 'https://example.com/gen/8')
    assert.equal(mode.getGuideLink(), null)

    // 일정이 있는 위치로 돌아오면 그 위치의 일정이 다시 채택된다.
    await mode.loadReadingDetail(7, 'gen', 2)
    assert.equal(mode.isOffPlanDetail(), false)
    assert.deepEqual(mode.getTongdokProgress('gen', 2), { current: 2, total: 3, done: 2, isCurrentInRange: true, isComplete: false })
