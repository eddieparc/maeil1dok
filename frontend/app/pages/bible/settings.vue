<template>
  <div class="bible-page settings-page">
    ...
    <BottomSheet :model-value="isOpen" title="읽기 설정" class="reading-settings-sheet" @update:model-value="closeSheet">
      <template #header-extra>
        <AppButton class="done-btn" variant="ghost" size="sm" :loading="isSaving" @click="saveSettings">완료</AppButton>
      </template>
...
const hasChanges = computed(() => savedSettings.value !== '' && JSON.stringify(settings.value) !== savedSettings.value);
...
const saveSettings = async () => {
  ...
  try {
    ...
    // The store reports a failed sync by leaving lastSyncedAt unchanged.
    if (settingsStore.lastSyncedAt === lastSyncedAt) {
...
<style scoped>
.settings-page { background: var(--color-bg-primary); letter-spacing: var(--tracking-body); }
.preview-section { padding: 32px 20px 96px; color: var(--color-text-primary); }
.preview-label { font-size: 12px; font-weight: 600; color: var(--color-text-tertiary); }
.preview-section h1 { margin: 12px 0 20px; font-size: 22px; font-weight: 700; letter-spacing: var(--tracking-display); }
...
:global([data-theme="dark"]) .font-button.active, :global([data-theme="dark"]) .chip-btn[aria-pressed="true"] { background: var(--color-bg-card); border: 1.5px solid var(--color-accent-primary); }
