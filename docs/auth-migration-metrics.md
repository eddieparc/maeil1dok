...
## 403 이 사용자를 로그아웃시키고 있었다 (2026-08-30 오전, 수정·배포)

서버 수정을 배포한 뒤 **같은 날 10:00 KST 에 iOS 사용자가 다시 로그아웃**됐다.
07:22 로그인이었으니 access 쿠키(1시간)는 08:22 에 만료됐고, 이번에는 refresh 경로가
**실제로 실행된** 첫 시험이었다.

### 서버 로그가 시퀀스를 그대로 담고 있었다

```
08:34:07  refresh_401  403  cause=csrf          ← 배포 전
────────── 08:38 배포(7ec253cc) ──────────
08:40:42  auth/none    200  auth_refresh        ← 본문 토큰 경로: 성공
08:49:43  refresh_401  403  cause=csrf          ← 쿠키 단독 경로: 여전히 실패
09:28:59  refresh_401  403  cause=csrf
09:29:00  auth_logout                            ← 직후 로그아웃
10:01:08  refresh_401  403  cause=csrf
10:01:08  auth_logout  403                       ← 직후 로그아웃
10:01:14  login                                  ← 사용자가 다시 로그인
```

**본문 토큰 경로(셸)는 고쳐졌고, 쿠키 단독 경로(웹)가 남아 있었다.**

### 두 개의 별개 결함이 겹쳐 있었다

**(1) 웹이 403 을 확정 로그아웃으로 읽었다.**
`useAuthService.refreshToken()` 이 `status === 401 || status === 403` 을 한 가지로 묶어
둘 다 `performLogout()` 했다. 401 은 서버가 신원을 거부한 것이라 세션이 끝난 게 맞지만,
403 은 CSRF 핸드셰이크 실패이고 **세션에 대해 아무것도 말하지 않는다.**

**(2) 브리지로 들어온 웹은 CSRF 토큰을 가진 적이 없었다.**
셸은 네이티브로 로그인하고 `session/issue` → `session/consume` 으로 세션만 웹뷰에 넘긴다.
그런데 `X-CSRFToken` 을 돌려주는 곳은 **로그인 응답과 refresh 성공뿐**이다. 웹은 로그인
응답을 한 번도 받지 못하므로 토큰이 아예 없는 상태로 시작하고, 5분 주기 refresh 타이머가
쿠키 단독 상환을 보내면 반드시 403 이다. 그리고 (1) 때문에 그것이 로그아웃이 됐다.

브라우저에서 직접 로그인한 사용자는 로그인 응답으로 토큰을 받으므로 영향이 없었다.
**셸 사용자만 5분마다 로그아웃 후보였다.**

### 수정

| 층 | 무엇을 |
|---|---|
| 웹 (`02c0f661`) | 403 → `/auth/csrf/` 로 토큰 재취득 후 **1회 재시도**. 그래도 실패하면 세션을 유지한 채 `unreachable`. **401 의 로그아웃은 그대로.** |
| API (`c6126f5b`) | `session_bridge_consume` 이 `get_token()` 으로 **CSRF 쿠키를 함께 심는다** → 첫 상환부터 헤더가 실려 403 이 애초에 안 난다. |

두 층을 다 둔 이유는 서로 다른 지점을 막기 때문이다. API 수정은 원인을 없애고, 웹 수정은
브리지를 거치지 않은 경로나 토큰이 만료된 경우를 받는다.

토큰 재취득은 채우기가 아니라 **덮어쓰기**다. `getCsrfToken()` 이 localStorage 를 먼저 읽으므로
낡은 값이 신선한 쿠키를 가리면 영구히 불일치한다.

### 프로덕션 실증 (배포 전)

```
[1] GET /auth/csrf/  →  {"csrfToken":"BlbB9zyn..."}
    Set-Cookie: csrftoken=...; Domain=.maeil1dok.app; SameSite=Lax; Secure
[2] 그 토큰으로 쿠키 단독 상환  →  401 "Invalid or expired refresh token"
```

`403 csrf` 가 아니라 **401** — CSRF 를 통과해 토큰 검증까지 도달했다. 회복 수단이 실재함을
자격증명 없이 증명한다.

### 또 테스트가 결함을 고정하고 있었다

`frontend/tests/auth-session-safety.test.mjs` 의 한 케이스가 `useAuthService` 소스를 스캔해

```
/result\.status === 401 \|\| result\.status === 403[\s\S]{0,300}reason: 'rejected'/
```

를 **요구**하고 있었다. 그 줄이 바로 10:00 로그아웃을 만든 코드다. `.json` 별칭의 500 을
기대값으로 승인하던 골든과 **같은 모양**이다 — 구현 위치를 문자열로 못박은 단언은 결함이
자리를 잡으면 그 결함을 지킨다. 동작(정책 함수 호출 결과)을 단언하도록 다시 썼다.

### CORS 는 원인이 아니었다 (후보 소거 기록)

의심했던 `X-CSRFToken` 응답 헤더 미노출은 아니었다. 프로덕션 응답에
`access-control-expose-headers: x-csrftoken` 과 `access-control-allow-headers: ... x-csrftoken`
이 모두 있다. 후보를 소거한 근거를 남겨 다음 사람이 같은 곳을 다시 파지 않게 한다.
